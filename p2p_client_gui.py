...
def listen_for_peers(host, port, stop_event):
    ...
    try:
        ...
        while running and not stop_event.is_set():
            try:
                ...
                if running and not stop_event.is_set():
                    gui_log(
                        f"P2P LISTENER CRITICAL ERROR] Unexpected error: {e}",
                        level="CRITICAL",
                    )
                break
    except OSError as e:
        ...
        if running and not stop_event.is_set():
            gui_log(
                f"P2P LISTENER BIND ERROR] Could not bind to {host}:{port} - {e}. Try a different port or check permissions.",
                level="ERROR",
            )
    except Exception as e:
        if running and not stop_event.is_set():
        ...
        if p2p_server_socket:
            try:
                ...
                gui_log(
                    f"P2P LISTENER WARN] Error closing listener socket: {e}",
                    level="WARN",
                )
...
def handle_peer_connection(peer_socket, peer_address):
    ...
    gui_log(
        f"P2P Connection established with {actual_peer_name} ({peer_address[0]}:{peer_address[1]})"
    )
    ...
    try:
        while running:  # Check global running flag
            ...
            try:
                ...
                gui_log(
                    f"P2P Received non-UTF8 data from {actual_peer_name}. Discarding buffer.",
                    level="ERROR",
                )
            ...
            while "\n" in buffer and running:
                ...
                try:
                    ...
                    if msg_type == "request_join_channel":
                        ...
                        gui_log(
                            f"P2P RECV] Join request for '{req_channel}' from '{req_name}'"
                        )

                        # Validate request data
                        if not (req_channel and req_name and req_ip and req_port):
                            gui_log(
                                f"P2P WARNING] Invalid join request from {actual_peer_name}: Missing fields",
                                level="WARN",
                            )
                            continue
                        ...
                        with my_channels_lock:
                            if (
                                req_channel in my_channels
                                and my_channels[req_channel].get("owner") == MY_NAME
                            ):
                                channel_data = my_channels[req_channel]
                                members_dict = channel_data.setdefault(
                                    "members", {}
                                )  # Get or create members dict
                                ...
                                if req_name not in members_dict:
                                    print(
                                        f"CHANNEL] Adding '{req_name}' to '{req_channel}'."
                                    )
                                else:
                                    print(
                                        f"CHANNEL INFO] '{req_name}' re-requested join to '{req_channel}'. Updating info."
                                    )
                                ...
                                threading.Thread(
                                    target=send_p2p_message,
                                    args=(req_name, json.dumps(accept_response) + "\n"),
                                    daemon=True,
                                ).start()
                                print(
                                    f"P2P >>] Sent join acceptance for '{req_channel}' to '{req_name}'."
                                )
                                # Notify GUI to update channel/member lists
                                gui_queue.put(
                                    ("UPDATE_MY_CHANNELS", my_channels.copy())
                                )

                            elif req_channel in my_channels:
                                gui_log(
                                    f"P2P WARNING] Join request for '{req_channel}' from '{req_name}', but not owner ({my_channels[req_channel].get('owner', '?')}). Ignoring.",
                                    level="WARN",
                                )
                            ...
                            # threading.Thread(target=send_p2p_message, args=(req_name, json.dumps(reject_response) + '\n'), daemon=True).start()
                                ...
                                gui_log(
                                    f"P2P WARNING] Join request from '{req_name}' for unknown local channel '{req_channel}'. Ignoring.",
                                    level="WARN",
                                )
                    ...
                    # --- Join Acceptance ---
                        ...
                        accepted_members = message.get(
                            "members", {}
                        )  # Owner sends the member list

                        gui_log(
                            f"P2P RECV] Successfully joined channel '{accepted_channel}' (Owner: {accepted_owner})!"
                        )
                        gui_log(
                            f"CHANNEL] Members in '{accepted_channel}': {list(accepted_members.keys())}"
                        )
                    ...
                    # --- Join Error ---
                        ...
                        gui_log(
                            f"P2P RECV] Join Failed for '{failed_channel}': {reason}",
                            level="ERROR",
                        )
                    ...
                    # --- Channel Message ---
                        ...
                        if channel and sender and content:
                            ...
                            if is_member:
                                ...
                                display_sender = (
                                    f"{sender} (You)" if sender == MY_NAME else sender
                                )
                                ...
                                gui_log(
                                    f"P2P WARNING] Received message for channel '{channel}' but not a member. Ignoring.",
                                    level="WARN",
                                )
                        else:
                            gui_log(
                                f"P2P WARNING] Received incomplete channel message from {actual_peer_name}",
                                level="WARN",
                            )
                    ...
                    # --- Forward Request (Received by Owner) ---
                        ...
                        if not (
                            channel and original_sender and msg_content is not None
                        ):
                            gui_log(
                                f"P2P WARNING] Invalid 'forward_to_owner' message from {actual_peer_name}: Missing fields",
                                level="WARN",
                            )
                            continue
                        ...
                        with my_channels_lock:
                            if (
                                channel in my_channels
                                and my_channels[channel].get("owner") == MY_NAME
                            ):
                                is_owner_of_channel = True
                                # Get a copy of members to avoid holding lock while sending
                                members_to_forward = (
                                    my_channels[channel].get("members", {}).copy()
                                )

                        if is_owner_of_channel:
                            gui_log(
                                f"P2P RECV] Received message from '{original_sender}' for owned channel '{channel}'. Forwarding..."
                            )
                            ...
                            with channel_logs_lock:
                                ...
                                channel_message_logs.setdefault(channel, []).append(
                                    log_entry
                                )
                            ...
                            for member_name, member_info in members_to_forward.items():
                                if (
                                    member_name != MY_NAME
                                    and member_name != original_sender
                                ):
                                    threading.Thread(
                                        target=send_p2p_message,
                                    ...
                                    send_count += 1
                            print(
                                f"P2P >>] Forwarded message from '{original_sender}' to {send_count} members of '{channel}'."
                            )

                            # 5. Send to registry for storage/potential offline delivery
                            if registry_socket and not is_offline:
                                try:
                                    registry_socket.sendall(
                                        forward_json.encode("utf-8")
                                    )  # Note: using forward_json here
                                    print(
                                        f"P2P >>] Sent forwarded message for '{channel}' to registry."
                                    )
                                except (socket.error, OSError) as e:
                                    gui_log(
                                        f"P2P ERROR] Failed to send forwarded message to registry: {e}",
                                        level="ERROR",
                                    )
                            else:
                                gui_log(
                                    f"P2P INFO] Not sending forwarded message to registry (offline or no socket)."
                                )

                        else:
                            # Received forward request but not the owner (or channel unknown)
                            gui_log(
                                f"P2P WARNING] Received 'forward_to_owner' from {actual_peer_name} for '{channel}', but not the owner or channel unknown. Ignoring.",
                                level="WARN",
                            )
                        ...
                        gui_log(
                            f"P2P UNHANDLED from {actual_peer_name}]: Type: {msg_type}, Content: {message_json}",
                            level="WARN",
                        )

                except json.JSONDecodeError:
                    gui_log(
                        f"P2P ERROR] Invalid JSON received from {actual_peer_name}: {message_json}",
                        level="ERROR",
                    )
                    ...
                    gui_log(
                        f"P2P ERROR] Error processing message from {actual_peer_name}: {e}",
                        level="ERROR",
                    )
                    import traceback
        ...
        if running:  # Avoid logging errors during normal shutdown
            gui_log(
                f"P2P HANDLER CRITICAL] Error in connection handler for {actual_peer_name}: {e}",
                level="CRITICAL",
            )
    finally:
...
def listen_to_registry(reg_socket, stop_event):
    ...
    while running and not stop_event.is_set():
        try:
            ...
            while "\n" in buffer:
                ...
                try:
                    ...
                    if msg_type == "peer_list":
                        ...
                        sorted_peer_list = sorted(
                            known_peers.values(),
                            key=lambda p: p.get("name", "z").lower(),
                        )
                        if sorted_peer_list:
                            for p_info in sorted_peer_list:
                                ...
                                status = p_info.get("login_status", "?")
                                status_str = f"({status})"
                    ...
                    # --- Peer Joined ---
                        ...
                        is_invisible_peer = (
                            info.get("is_invisible", False) if info else False
                        )
                        ...
                        if peer_id:
                            ...
                            with peer_list_lock:
                                if peer_id in known_peers:
                                    removed_name = known_peers.pop(peer_id).get(
                                        "name", "Unknown"
                                    )
                    ...
                    # --- Channel Created Confirmation ---
                        ...
                        gui_log(
                            f"REGISTRY] Channel '{c_name}' creation confirmed (Owner: {c_owner})."
                        )
                        ...
                        if c_owner == MY_NAME:
                            with my_channels_lock:
                                if c_name not in my_channels:
                                    ...
                                    gui_log(
                                        f"CHANNEL] Added owned channel '{c_name}' to local list."
                                    )
                    ...
                    # --- Channel Error ---
                        ...
                        gui_log(
                            f"REGISTRY ERROR] Channel '{c_name}': {error_msg}",
                            level="ERROR",
                        )
                    ...
                    # --- New Channel Available ---
                        ...
                        gui_log(
                            f"REGISTRY] New channel available: '{c_name}' (Owner: {c_owner}, Type: {c_type})"
                        )
                    ...
                    # --- Full Channel List ---
                        ...
                        gui_log(
                            "REGISTRY RECV] Received list of all available channels."
                        )
                        channels_data = message.get(
                            "channels", {}
                        )  # Expect dict {name: info}
                        with available_channels_info_lock:
                            available_channels_info = (
                                channels_data  # Replace local list
                            )
                    ...
                    # ... (previous message type handlers) ...
                        ...
                        received_messages = message.get(
                            "messages"
                        )  # List of message dicts
                        return_status = message.get(
                            "status"
                        )  # 'sync_point_found', 'sync_point_not_found', etc.

                        # Basic validation
                        if (
                            not channel_name
                            or received_messages is None
                            or return_status is None
                        ):
                            # Use print for errors in background threads, or a dedicated error queue
                            print(
                                f"[SYNC REPLY ERROR] Received incomplete sync_reply: {message}"
                            )
                        ...
                        print(
                            f"[SYNC REPLY] Received reply for '{channel_name}', status: '{return_status}', messages: {len(received_messages)}"
                        )
                        ...
                        if return_status == "sync_point_not_found":
                            if not received_messages:
                                print(
                                    f"[SYNCHRONIZATION] Server didn't find sync point for '{channel_name}', no history provided."
                                )
                            else:
                                print(
                                    f"[SYNCHRONIZATION] Server didn't find sync point for '{channel_name}', receiving full log ({len(received_messages)} messages)."
                                )
                        elif return_status == "sync_point_found":
                            if not received_messages:
                                print(
                                    f"[SYNCHRONIZATION] Sync point found for '{channel_name}', no newer messages."
                                )
                                continue  # Nothing more to do for this message
                            else:
                                print(
                                    f"[SYNCHRONIZATION] Sync point found for '{channel_name}', receiving {len(received_messages)} new/subsequent messages."
                                )
                        else:
                            print(
                                f"[SYNC REPLY WARNING] Received unknown status '{return_status}' for channel '{channel_name}'"
                            )
                        ...
                        if received_messages:
                            ...
                            with my_channels_lock:  # Check ownership safely
                                ...
                                if (
                                    channel_info
                                    and channel_info.get("owner") == MY_NAME
                                ):
                                    is_owner_of_channel = True

                            if is_owner_of_channel:
                                with channel_logs_lock:
                                    log_for_channel = channel_message_logs.setdefault(
                                        channel_name, []
                                    )
                                    # Add only messages not already present (basic timestamp check)
                                    existing_timestamps = {
                                        msg["timestamp"] for msg in log_for_channel
                                    }
                                    new_messages_to_add = []
                                    ...
                                    if new_messages_to_add:
                                        ...
                                        try:
                                            ...
                                            print(
                                                f"[SYNC WARN] Could not sort persistent log for '{channel_name}' after sync: {e}"
                                            )

                                if messages_added_to_log_count > 0:
                                    print(
                                        f"[SYNC INFO] Added {messages_added_to_log_count} unique messages to persistent log for '{channel_name}'."
                                    )

                            # 2. Format and Queue ALL received messages for GUI Display / Buffering
                            # This happens regardless of ownership
                            print(
                                f"[SYNC INFO] Queuing {len(received_messages)} received messages for GUI display/buffer for '{channel_name}'."
                            )
                            for msg_data in received_messages:
                                try:
                                    ...
                                    if (
                                        ...
                                        print(
                                            f"[SYNC WARN] Skipping synced message in '{channel_name}' due to missing timestamp: {msg_data}"
                                        )
                                        continue
                                    ...
                                    gui_queue.put(
                                        ("CHANNEL_MSG", channel_name, formatted_msg)
                                    )

                                except Exception as e:
                                    # Log errors during formatting/queuing for individual messages
                                    print(
                                        f"[SYNC ERROR] Error processing synced message for GUI queue: {e} - Data: {msg_data}"
                                    )
                                    import traceback
                    ...
                    # --- Notify New Message (Real-time push from Server) ---
                        ...
                        if (
                            ...
                            or sender is None
                            or content is None
                            or timestamp_str is None
                        ):
                            gui_log(
                                f"NOTIFY ERROR] Incomplete 'notify_new_msg' received: {message}",
                                level="ERROR",
                            )
                            continue
                        ...
                        if is_member:
                            ...
                            formatted_msg = (
                                f"[{channel_name} @ {time_str}] {sender}: {content}"
                            )
                            ...
                            with my_channels_lock:
                                if (
                                    channel_name in my_channels
                                    and my_channels[channel_name].get("owner")
                                    == MY_NAME
                                ):
                                    is_owner_of_channel = True
                            ...
                            if is_owner_of_channel:
                                with channel_logs_lock:
                                    ...
                                    log_list = channel_message_logs.setdefault(
                                        channel_name, []
                                    )
                                    # Simple check: don't add if last message is identical
                                    if (
                                        not log_list
                                        or log_list[-1]["timestamp"] != timestamp_str
                                        or log_list[-1]["sender"] != sender
                                        or log_list[-1]["content"] != content
                                    ):
                    ...
                    # --- Unknown Message Type ---
                        ...
                        gui_log(
                            f"REGISTRY UNHANDLED] Type: {msg_type}, Content: {message_json}",
                            level="WARN",
                        )

                except json.JSONDecodeError:
                    gui_log(
                        f"REGISTRY ERROR] Invalid JSON received: {message_json}",
                        level="ERROR",
                    )
                except Exception as e:
                    gui_log(
                        f"REGISTRY ERROR] Error processing message: {e}\nMessage: {message_json}",
                        level="ERROR",
                    )
                    import traceback
            ...
            if running and not stop_event.is_set():
                gui_log(
                    f"REGISTRY LISTENER ERROR] Connection error: {e}", level="ERROR"
                )
            ...
            if running and not stop_event.is_set():
                gui_log(
                    f"REGISTRY LISTENER CRITICAL] Unexpected error: {e}",
                    level="CRITICAL",
                )
            break  # Exit loop
...
def send_p2p_message(target_name, message_json_with_newline):
    ...
    if is_offline:
        gui_log(
            f"P2P SEND ERROR] Cannot send to '{target_name}': You are offline.",
            level="ERROR",
        )
        return False
    ...
    if found_in_registry and target_is_offline_in_registry:
        print(
            f"[P2P SEND INFO] Cannot send to '{target_name}': Peer is reported as offline by the registry in 'known_peers'."
        )
    ...
    if not ip or not port:
        print(
            f"P2P SEND ERROR] Peer '{target_name}' has incomplete info (IP/Port missing).",
            level="ERROR",
        )
        return False
    ...
    try:
        if found_in_registry:
            ...
            print(
                f"[P2P SEND OK] Message sent successfully to '{target_name}'."
            )  # Maybe too verbose
            return True
        else:
        ...
        gui_log(
            f"P2P SEND ERROR] Connection to '{target_name}' ({ip}:{port}) timed out.",
            level="ERROR",
        )
    except ConnectionRefusedError:
        gui_log(
            f"P2P SEND ERROR] Connection to '{target_name}' ({ip}:{port}) refused. Peer might be offline or listener down.",
            level="ERROR",
        )
        ...
        gui_log(
            f"P2P SEND ERROR] Network error sending to '{target_name}' ({ip}:{port}): {e}",
            level="ERROR",
        )
    except Exception as e:
        gui_log(
            f"P2P SEND ERROR] Unexpected error sending to '{target_name}': {e}",
            level="ERROR",
        )
        import traceback
...
def create_channel(reg_socket, channel_name, channel_type):
    ...
    if not reg_socket or is_offline:  # Check offline status too
        gui_log(
            "ERROR] Cannot create channel: Not connected to registry (Offline?).",
            level="ERROR",
        )
        return False
    ...
    if not channel_name or not channel_type or " " in channel_name:
        gui_log(
            "ERROR] Invalid channel name (no spaces allowed) or type.", level="ERROR"
        )
        return False
    ...
    try:
        ...
        gui_log(
            f"CMD] Channel creation request sent for '{channel_name}' ({channel_type})."
        )
        return True
    except (socket.error, OSError) as e:
        gui_log(
            f"ERROR] Network error sending create channel request: {e}", level="ERROR"
        )
        return False
    except Exception as e:
...
def join_channel(channel_name_to_join):
    ...
    with my_channels_lock:
        if channel_name_to_join in my_channels:
            role = (
                "owner"
                if my_channels[channel_name_to_join].get("owner") == MY_NAME
                else "member"
            )
    ...
    if not owner_info:
        gui_log(
            f"JOIN ERROR] Channel '{channel_name_to_join}' not found in available list. Use 'List Channels' first.",
            level="ERROR",
        )
        return False
    ...
    if not owner_name or not owner_ip or not owner_port:
        gui_log(
            f"JOIN ERROR] Incomplete owner info for '{channel_name_to_join}' in available list.",
            level="ERROR",
        )
        return False
    ...
    if not MY_INFO or not MY_NAME:
        gui_log(
            "JOIN ERROR] Cannot join: Your own info (IP/Port) is missing.",
            level="ERROR",
        )
        return False
    ...
    gui_log(
        f"JOIN] Sending P2P join request for '{channel_name_to_join}' to owner '{owner_name}'..."
    )
    ...
    if success:
        ...
        gui_log(
            f"JOIN ERROR] Failed to send P2P join request to owner '{owner_name}'. They might be offline or unreachable.",
            level="ERROR",
        )
...
def send_channel_msg(channel_name, msg_content, reg_socket):
    ...
    if not channel_name or not msg_content:
        gui_log(
            "MSG ERROR] Channel name and message content cannot be empty.",
            level="ERROR",
        )
        return False
    ...
    if is_offline:
        ...
        formatted_msg = (
            f"[{channel_name} @ {time_str}] {MY_NAME} (Offline Queued): {msg_content}"
        )
        gui_queue.put(("CHANNEL_MSG", channel_name, formatted_msg))
        gui_log(
            f"MSG OFFLINE QUEUED] Message for '{channel_name}' queued for sending when online."
        )
        return True
    ...
    with my_channels_lock:
        if channel_name not in my_channels:
            gui_log(
                f"MSG ERROR] Not a member of channel '{channel_name}'. Cannot send.",
                level="ERROR",
            )
            return False
    ...
    if not MY_NAME or not MY_INFO:
        gui_log(
            "MSG ERROR] Cannot send message: User identity info missing.", level="ERROR"
        )
        return False

    timestamp_iso = datetime.datetime.now(
        timezone.utc
    ).isoformat()  # Timestamp for online send

    if is_owner:
        # --- Owner Sending (Online) ---
        gui_log(
            f"MSG SENDING] Broadcasting message as owner to '{channel_name}' members..."
        )
        message_payload = {
        ...
        for member_name in members_dict:
            ...
            if member_name != MY_NAME:  # Optionally skip self
                threading.Thread(
                    target=send_p2p_message,
                    args=(member_name, message_json),
                    daemon=True,
                ).start()
                send_count += 1
        gui_log(
            f"MSG SENT] Broadcast attempt to {send_count} members of '{channel_name}'."
        )
        ...
        if reg_socket:
            try:
                ...
                gui_log(
                    f"MSG ERROR] Failed to send message to registry: {e}", level="ERROR"
                )
        else:
            gui_log(
                "MSG WARN] Cannot send message to registry: Socket unavailable.",
                level="WARN",
            )
        ...
        if not owner_name:
            ...
            gui_log(
                f"MSG ERROR] Cannot send message to '{channel_name}': Owner name unknown.",
                level="ERROR",
            )
            return False

        gui_log(
            f"MSG SENDING] Forwarding message for '{channel_name}' to owner '{owner_name}'..."
        )
        ...
        if p2p_success:
            gui_log(
                f"MSG SENT] Successfully forwarded message via P2P to owner '{owner_name}'."
            )
            ...
            gui_log(
                f"MSG WARNING] Failed P2P send to owner '{owner_name}'. Attempting to send to server log...",
                level="WARN",
            )
            if reg_socket:
                try:
                    ...
                    try:
                        ...
                        time_str = "time?"
                    formatted_msg = (
                        f"[{channel_name} @ {time_str}] {MY_NAME}: {msg_content}"
                    )
                    gui_queue.put(("CHANNEL_MSG", channel_name, formatted_msg))

                except (socket.error, OSError) as e:
                    gui_log(
                        f"MSG ERROR] Failed to send message to server log after P2P failure: {e}",
                        level="ERROR",
                    )
                ...
                gui_log(
                    "MSG ERROR] Failed P2P to owner and cannot reach server (offline?). Message not sent.",
                    level="ERROR",
                )
                return False  # Indicate failure
...
def synchronize_host_server(reg_socket):
    ...
    if is_offline or not reg_socket:
        gui_log(
            "OWNER SYNC SERVER] Cannot sync: Offline or no registry socket.",
            level="WARN",
        )
        return
    ...
    with my_channels_lock:
        owned_channels_to_check = [
            name for name, data in my_channels.items() if data.get("owner") == MY_NAME
        ]
    ...
    gui_log(
        f"OWNER SYNC SERVER] Checking {len(owned_channels_to_check)} owned channels for synchronization..."
    )
    ...
    for channel_name in owned_channels_to_check:
        ...
        if not content:
            gui_log(
                f"[OWNER SYNC SERVER] Owned channel '{channel_name}' has no local messages."
            )
            continue
        ...
        if channel_latest_timestamp:
            gui_log(
                f"[OWNER SYNC SERVER] Latest local message for owned channel '{channel_name}' (ts: {channel_latest_timestamp}):"
            )
            ...
            try:
                ...
                gui_log(
                    f"[SYNC SEND] Sent sync request for '{channel_name}' with timestamp {channel_latest_timestamp}"
                )
            except Exception as e:
                gui_log(
                    f"[SYNC ERROR] Failed to send sync request for '{channel_name}': {e}"
                )

        else:
            gui_log(
                f"[OWNER SYNC SERVER WARN] Latest message in owned channel '{channel_name}' lacks a 'timestamp': {channel_latest_message}"
            )
...
def synchronize_offline_messages(reg_socket):
    ...
    if is_offline or not reg_socket:
        gui_log(
            "SYNC ERROR] Cannot process offline queue: Client is still offline or registry unavailable.",
            level="ERROR",
        )
    ...
    for msg_data in messages_to_process:
        ...
        content = msg_data.get("content")
        original_timestamp = msg_data.get(
            "timestamp"
        )  # Use the timestamp from when it was typed

        if not channel_name or content is None or not original_timestamp:
            gui_log(
                f"SYNC ERROR] Invalid queued data skipped: {msg_data}", level="ERROR"
            )
            failed_sends += 1
            continue
        ...
        with my_channels_lock:
            ...
            if channel_info:
                ...
                gui_log(
                    f"SYNC ERROR] Cannot send queued message for '{channel_name}': No longer in channel. Discarded.",
                    level="ERROR",
                )
                failed_sends += 1
        ...
        if is_currently_owner:
            ...
            with channel_logs_lock:
                ...
                if (
                    not log_list
                    or log_list[-1]["timestamp"] != original_timestamp
                    or log_list[-1]["sender"] != MY_NAME
                    or log_list[-1]["content"] != content
                ):
                    log_list.append(log_entry)
                    # Sort?
            ...
            for member_name in members_dict:
                ...
                send_count += 1
            gui_log(
                f"SYNC] Broadcast attempt for queued message to {send_count} members of '{channel_name}'."
            )
            ...
            try:
                ...
                reg_socket.sendall(registry_json.encode("utf-8"))
                gui_log(
                    f"SYNC] Sent queued owner message for '{channel_name}' to registry."
                )
                message_sent = True  # Assume success if sent to registry
            except (socket.error, OSError) as e:
                gui_log(
                    f"SYNC ERROR] Failed to send queued owner message to registry: {e}",
                    level="ERROR",
                )
            ...
            if not owner_name:  # Should have owner if member, but double check
                gui_log(
                    f"SYNC ERROR] Cannot send queued member message for '{channel_name}': Owner unknown. Discarded.",
                    level="ERROR",
                )
                failed_sends += 1
                continue
            ...
            gui_log(
                f"SYNC] Forwarding queued member message for '{channel_name}' to owner '{owner_name}'..."
            )
            message_payload = {
            ...
            if p2p_success:
                gui_log(
                    f"SYNC] Successfully forwarded queued message via P2P to owner '{owner_name}'."
                )
                message_sent = True
            else:
                # P2P failed, try registry backup
                gui_log(
                    f"SYNC WARNING] Failed P2P sync to owner '{owner_name}'. Sending to server log...",
                    level="WARN",
                )
                try:
                    ...
                    reg_socket.sendall(message_json.encode("utf-8"))
                    gui_log(
                        "SYNC INFO] Queued message forwarded to server log as P2P backup."
                    )
                    message_sent = True  # Count as success if registry gets it
                except (socket.error, OSError) as e:
                    gui_log(
                        f"SYNC ERROR] Failed to send queued message to server log: {e}. Discarded.",
                        level="ERROR",
                    )
        ...
        if message_sent:
            ...
            failed_sends += 1
            gui_log(
                f"SYNC ERROR] Failed to send queued message for '{channel_name}'.",
                level="ERROR",
            )

    # End of loop
    gui_log(
        f"SYNC] Finished processing offline queue. Success: {successful_sends}, Failed/Discarded: {failed_sends}."
    )
...
def attempt_login_or_guest(host, port, login_details):
    ...
    try:
        ...
        gui_log(
            f"CONNECTED] Connection established. Sending identification as '{name}'..."
        )
        ...
        while not ack_received:
            try:
                ...
                if not data:
                    final_message = (
                        "Connection closed by server before acknowledgement."
                    )
                ...
                if "\n" in buffer:
                    ...
                    if r_type == "login_ack" or r_type == "guest_ack":
                        ...
                        final_message = response.get(
                            "message", "Login failed by server."
                        )
                        success = False
                        ...
                        final_message = (
                            f"Unexpected response type from server: {r_type}"
                        )
                        success = False
...
def attempt_reconnect_and_register(host, port):
    ...
    try:
        ...
        buffer = ""
        ack_received = False
        final_message = "No reconnect response."
        success = False
            ...
            try:
                ...
                if "\n" in buffer:
                    ...
                    if r_type == "login_ack" or r_type == "guest_ack":
                        ...
                        final_message = (
                            f"Unexpected ack type during reconnect: {r_type}"
                        )
                        success = False
                        break
            except socket.timeout:
...
class P2PClientGUI(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        peer_scroll = tk.Scrollbar(
            left_pane, orient=tk.VERTICAL, command=self.peer_listbox.yview
        )
        ...
        tk.Label(left_pane, text="My Channels:").grid(
            row=2, column=0, sticky="w", pady=(10, 0)
        )
        self.my_channels_listbox = tk.Listbox(
            left_pane, height=6, exportselection=False
        )
        self.my_channels_listbox.grid(row=3, column=0, sticky="nsew")
        my_channels_scroll = tk.Scrollbar(
            left_pane, orient=tk.VERTICAL, command=self.my_channels_listbox.yview
        )
        ...
        tk.Label(left_pane, text="Available Channels:").grid(
            row=4, column=0, sticky="w", pady=(10, 0)
        )
        self.avail_channels_listbox = tk.Listbox(
            left_pane, height=6, exportselection=False
        )
        self.avail_channels_listbox.grid(row=5, column=0, sticky="nsew")
        avail_channels_scroll = tk.Scrollbar(
            left_pane, orient=tk.VERTICAL, command=self.avail_channels_listbox.yview
        )
        ...
        self.avail_channels_listbox.bind(
            "<Double-Button-1>", self._on_avail_channel_doubleclick
        ...
        self.chat_area = scrolledtext.ScrolledText(
            right_pane, wrap=tk.WORD, state="disabled", height=20
        )
        ...
        )  # Might not be used in chat anymore
        ...
        self.chat_area.tag_configure(
            ...
            foreground="red",
            font=(
                self.default_font.actual()["family"],
                self.default_font.actual()["size"],
                "bold",
            ),
        )
        self.chat_area.tag_configure(
            "CHANNEL_MSG", foreground="black"
        )  # Default for channel messages
        self.chat_area.tag_configure(
            "SELF_MSG", foreground="#008800"
        )  # Darker Green for own messages
        ...
        self.chat_area.tag_configure(
            "SYSTEM", foreground="purple"
        )  # Optional for system messages shown
        ...
        self.send_button = tk.Button(
            right_pane, text="Send", command=self._send_message_or_command, width=10
        )
        ...
        self.list_peers_button = tk.Button(
            button_frame, text="List Peers", command=self._list_peers
        )
        self.list_peers_button.pack(side=tk.LEFT, padx=2)
        self.list_channels_button = tk.Button(
            button_frame, text="List Channels", command=self._list_channels
        )
        self.list_channels_button.pack(side=tk.LEFT, padx=2)
        self.create_channel_button = tk.Button(
            button_frame, text="Create", command=self._create_channel_dialog
        )
        self.create_channel_button.pack(side=tk.LEFT, padx=2)
        self.join_channel_button = tk.Button(
            button_frame, text="Join", command=self._join_channel_dialog
        )
        self.join_channel_button.pack(side=tk.LEFT, padx=2)
        self.visibility_button = tk.Button(
            button_frame, text="Go Invisible", command=self._toggle_visibility
        )
        self.visibility_button.pack(side=tk.LEFT, padx=2)
        self.online_offline_button = tk.Button(
            button_frame, text="Go Online", command=self._toggle_online_offline
        )
        ...
        self.status_bar = tk.Label(
            bottom_pane, textvariable=self.status_var, relief=tk.SUNKEN, anchor=tk.W
        )
    ...
    def _show_login_dialog(self):
        ...
        tk.Label(frame, text="User ID (Login only):").grid(
        ...
        tk.Label(frame, textvariable=self.login_status_var, fg="red").grid(
        ...
        guest_button = tk.Button(
            button_frame, text="Login as Guest", command=self._handle_guest_login
        )
        guest_button.pack(side=tk.LEFT, padx=10)
        login_button = tk.Button(
            button_frame, text="Login", command=self._handle_user_login
        )
        login_button.pack(side=tk.LEFT, padx=10)

        self.login_window.protocol(
            "WM_DELETE_WINDOW", self._quit_app
        )  # Close app if login cancelled
    ...
    def _handle_guest_login(self):
        ...
        threading.Thread(
            target=self._execute_login, args=(login_details,), daemon=True
        ).start()
    ...
    def _execute_login(self, login_details):
        ...
        global is_offline
        success, message, sock = attempt_login_or_guest(
            REGISTRY_HOST, REGISTRY_PORT, login_details
        )
    ...
    def _start_network_listeners(self, reg_sock):
        ...
        p2p_listener_thread = threading.Thread(
            target=listen_for_peers,
            args=(LISTEN_HOST, MY_P2P_PORT, stop_p2p_listener_event),
            daemon=True,
        )
        ...
        if reg_sock:
            ...
            registry_listener_thread = threading.Thread(
                target=listen_to_registry,
                args=(reg_sock, stop_registry_listener_event),
                daemon=True,
            )
            registry_listener_thread.start()
        else:
            self.log_message(
                "CRITICAL: Registry socket unavailable after login. Forcing offline.",
                level="CRITICAL",
            )
    ...
    def _process_gui_queue(self):
        ...
        try:
            while True:  # Process all messages currently in queue
                ...
                if isinstance(item, str):
                    ...
                    if msg_lower.startswith("[error") or msg_lower.startswith("error"):
                        level = "ERROR"
                    elif msg_lower.startswith("[warn") or msg_lower.startswith("warn"):
                        level = "WARN"
                    elif msg_lower.startswith("[critical") or msg_lower.startswith(
                        "critical"
                    ):
                        level = "CRITICAL"
                    elif (
                        msg_lower.startswith("[channel")
                        or "joined channel" in msg_lower
                        or "left channel" in msg_lower
                    ):
                        level = "JOIN_PART"
                    elif msg_lower.startswith("[info") or msg_lower.startswith("info"):
                        level = "INFO"
                    ...
                    # --- Other tuple handlers remain the same ---
                        ...
                        if (
                            ...
                            and self.current_channel not in my_channels_data
                        ):
                            self.log_message(
                                f"You are no longer in channel '{self.current_channel}'.",
                                level="WARN",
                            )
                            ...
                            self.chat_area.insert(
                                tk.END,
                                "Select a channel from 'My Channels'.\n",
                                "SYSTEM",
                            )
                            self.chat_area.config(state="disabled")
                            self._set_ui_state(
                                logged_in=True, online=(not is_offline)
                        ...
                        self.online_offline_button.config(
                            state=tk.NORMAL if initial_login_info else tk.DISABLED
                        )

                    elif msg_type == "LOGIN_SUCCESS":
                        self.log_message(
                            f"Welcome, {'Guest ' if is_guest else ''}{MY_NAME}!",
                            level="SYSTEM",
                        )
                        self._update_status_bar()
            ...
            traceback.print_exc()
            self.log_message(
                f"CRITICAL Error processing GUI events: {e}", level="CRITICAL"
            )
    ...
    def _display_channel_message(self, channel, formatted_msg):
        ...
        if channel == self.current_channel:
            ...
            if f"] {MY_NAME} " in formatted_msg and (
                formatted_msg.strip().endswith(":")
                or f"] {MY_NAME} (You):" in formatted_msg
                or f"] {MY_NAME} (Offline Queued):" in formatted_msg
            ):
                tag = "SELF_MSG"
    ...
    def _update_peer_list(self, peers_data):
        ...
        self.peer_listbox.delete(0, tk.END)
        sorted_peers = sorted(
            peers_data.values(), key=lambda p: p.get("name", "z").lower()
        )
        for p_info in sorted_peers:
            ...
            # Omit self from list
            if name == MY_NAME and p_port == MY_P2P_PORT:
                continue
            # Omit invisible peers
            if p_info.get("is_invisible", False):
                continue

            offline_status = "(Offline)" if p_info.get("is_offline", False) else ""
            entry = (
                f"{name} ({p_status}_{p_port})"  # ({p_ip}:{p_port})" # Keep it simple
            )
    ...
    def _update_my_channels_list(self, my_channels_data):
        ...
        for name, data in sorted_channels:
            ...
            owner_display = (
                f" (Owner: {data.get('owner', '?')})" if role == "Member" else ""
            )
        ...
        if new_selection_index != -1:
            try:
                ...
                print(
                    f"[WARN] Error re-selecting item at index {new_selection_index}: {e}"
                )
    ...
    def _on_my_channel_select(self, event):
        ...
        if not selection:
            ...
            ):  # Only update if a channel was previously selected
                self.current_channel = None
                self.chat_area.config(state="normal")
                self.chat_area.delete("1.0", tk.END)
                self.chat_area.insert(
                    tk.END, "Select a channel from 'My Channels'.\n", "SYSTEM"
                )
                self.chat_area.config(state="disabled")
        ...
        try:
            ...
            update_needed = (
                new_channel_name != self.current_channel
            ) or notification_cleared
            ...
            if update_needed:
                ...
                if channel_actually_changed:
                    ...
                    messages_to_display = self.channel_display_buffers.get(
                        self.current_channel, []
                    )
                    if not messages_to_display:
                        self.chat_area.insert(
                            tk.END,
                            f"Message history for '{self.current_channel}' (This session).\n",
                            "SYSTEM",
                        )
                    else:
                        for msg in messages_to_display:
                            tag = "CHANNEL_MSG"
                            # Detect self message
                            if f"] {MY_NAME} " in msg and (
                                msg.strip().endswith(":")
                                or f"] {MY_NAME} (You):" in msg
                                or f"] {MY_NAME} (Offline Queued):" in msg
                            ):
                                tag = "SELF_MSG"
                ...
                self.send_button.config(state=tk.NORMAL if can_type else tk.DISABLED)
                if can_type:
                    self.input_entry.focus_set()  # Set focus if enabled

                # --- Redraw channel list ONLY if notification was cleared ---
                if notification_cleared:
                    print(
                        f"[DEBUG] Redrawing channel list after clearing notification for {new_channel_name}"
                    )
                    # Get current channel data safely to pass to update function
                    with my_channels_lock:
                        current_my_channels = my_channels.copy()
                    self._update_my_channels_list(
                        current_my_channels
    ...
    def _on_avail_channel_doubleclick(self, event):
        ...
        if channel_name:
            if messagebox.askyesno(
                "Join Channel",
                f"Do you want to attempt to join channel '{channel_name}'?",
            ):
                self.log_message(f"Attempting to join channel '{channel_name}'...")
                # Run join in background thread
                threading.Thread(
                    target=join_channel, args=(channel_name,), daemon=True
                ).start()
    ...
    def _send_message_or_command(self, event=None):
        ...
        if input_text.startswith("/"):
            ...
            if command == "/list":
                ...
                self._create_channel_dialog(args)
            elif command == "/join":
                self._join_channel_dialog(args)
            elif command == "/my_channels":
                self._show_my_channels()
            elif command == "/members":
                self._show_channel_members(args)
            elif command == "/history":
                self._show_channel_history(args)
            elif command == "/quit":
                self._on_closing()
                ...
                self._handle_msg_command(args)
            elif command == "/clear":
                self.chat_area.config(state="normal")
                self.chat_area.delete("1.0", tk.END)
                self.chat_area.config(state="disabled")
                self.log_message("Chat log cleared.")
            elif command == "/help":
                self._show_help()

            else:
                self.log_message(f"Unknown command: {command}", level="ERROR")

        # --- Channel Message Sending ---
        elif (
            self.current_channel and initial_login_info
        ):  # Check login status via initial_login_info
            print(
                f"[DEBUG] GUI initiating send to {self.current_channel} (Offline: {is_offline})"
            )  # Console Debug
    ...
    def _handle_msg_command(self, args):
        ...
        if len(parts) == 2:
            ...
            if channel_name and message_content:
                if is_offline:
                    self.log_message(
                        f"Queueing message for {channel_name} via /msg (offline)..."
                    )
                ...
                self.log_message(
                    "Usage: /msg <channel_name> <message content>", level="ERROR"
                )
        else:
            self.log_message(
                "Usage: /msg <channel_name> <message content>", level="ERROR"
    ...
    def _list_peers(self):
        ...
        if is_offline or not registry_socket:
            self.log_message(
                "Cannot list peers: Offline or not connected.", level="ERROR"
            )
            return
    ...
    def _list_channels(self):
        ...
        if is_offline or not registry_socket:
            self.log_message(
                "Cannot list channels: Offline or not connected.", level="ERROR"
            )
            return
    ...
    def _create_channel_dialog(self, args=""):
        ...
        channel_type = simpledialog.askstring(
            "Create Channel",
            "Enter Channel Type (public/private):",
            initialvalue=prefill_type,
            parent=self,
        )
        if not channel_type:
            return  # User cancelled
        ...
        if channel_name and channel_type:
            ...
            self.log_message(
                f"Attempting to create channel '{channel_name}' ({channel_type})..."
            )
    ...
    def _toggle_visibility(self):
        ...
        self.visibility_button.config(
            text="Go Visible" if is_invisible else "Go Invisible"
        )
        ...
        if not is_offline and registry_socket:
            ...
            try:
                ...
                self.log_message(
                    f"Error updating visibility with server: {e}", level="ERROR"
                )
    ...
    def _go_offline_logic(self, notify_server=True):
        ...
        if notify_server and registry_socket:
            try:
                ...
                self.log_message(
                    f"Warning: Failed to send offline status notification: {e}",
                    level="WARN",
                )
        ...
        if registry_socket:
            ...
            try:
                ...
                self.log_message(
                    f"Warning: Error closing registry socket: {e}", level="WARN"
                )
            registry_socket = None
    ...
    def _go_online_logic(self):
        ...
        self._set_ui_state(
            logged_in=True, online=False
        )  # Keep UI mostly disabled during attempt
        self.online_offline_button.config(
            state=tk.DISABLED
    ...
    def _execute_reconnect(self):
        ...
        global is_offline, registry_socket
        success, message, sock = attempt_reconnect_and_register(
        ...
        if success:
            ...
            if sock:
                try:
                    ...
                    gui_queue.put(
                        f"[WARN] Failed to send online status update after reconnect: {e}"
                    )
            ...
            if sock:
                ...
                threading.Thread(
                    target=synchronize_host_server, args=(sock,), daemon=True
                ).start()
                time.sleep(0.1)  # Tiny delay between syncs
                threading.Thread(
                    target=synchronize_offline_messages, args=(sock,), daemon=True
                ).start()
        else:
    ...
    def _show_myinfo(self):
        ...
        info_str += (
            f"P2P Address: {MY_INFO.get('ip', '?')}:{MY_INFO.get('p2p_port', '?')}"
        )
    ...
    def _show_my_channels(self):
        ...
        if not my_channels:
            messagebox.showinfo(
                "My Channels", "You are not currently in any channels.", parent=self
            )
            return
    ...
    def _show_channel_members(self, channel_name_arg):
        ...
        if not channel_name:
            ...
            if selection:
                ...
                self.log_message(
                    "Usage: /members <channel_name> (or select from 'My Channels')",
                    level="ERROR",
                )
                return
        ...
        if not channel_data:
            self.log_message(
                f"You are not currently in channel '{channel_name}'.", level="ERROR"
            )
            return
    ...
    def _show_channel_history(self, channel_name_arg):
        ...
        if not channel_name:
            ...
            if selection:
                ...
                self.log_message(
                    "Usage: /history <channel_name> (requires ownership, or select from 'My Channels')",
                    level="ERROR",
                )
                return
        ...
        with my_channels_lock:
            is_owner = (
                channel_name in my_channels
                and my_channels[channel_name].get("owner") == MY_NAME
            )

        if not is_owner:
            self.log_message(
                f"Cannot show local history for '{channel_name}': You are not the owner.",
                level="ERROR",
            )
        ...
        text_area = scrolledtext.ScrolledText(
            history_window, wrap=tk.WORD, state="normal"
        )
    ...
    def _quit_app(self):
        ...
        if not is_offline and registry_socket:
            try:
                ...
                self.log_message(
                    f"Warning: Failed to send final offline status: {e}", level="WARN"
                )
